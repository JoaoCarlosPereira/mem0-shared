...
6. JWT de sessão UI → upsert `user_account` por e-mail (nome/foto/`language=pt-BR`/role admin) + **membership compartilhada** (project manager + board editor) em **todos** os projetos/quadros Spec. Ambiente ShareMem: todos veem e editam todos os quadros. Bearer INTERNAL continua DEFAULT_ADMIN (mirror FK-safe).
